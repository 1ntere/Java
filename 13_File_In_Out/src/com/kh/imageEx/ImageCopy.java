...
File In put Stream : 파일의 데이터를 읽어오는데 사용

	read() : 데이터를 읽어옴
	
File Out put Stream : 파일의 데이터를 내보내는데 사용
...
public class ImageCopy {
	public static void main(String[] args) {
		...
		// 모든 파일의 실행은 src로 부터 시작 함
		//				 폴더 경로를 모두 쓰고 나면 / 를 한 후 파일이름 작성
		String 원본이미지 = "src/com/kh/imageEx/healing.jpg";//ctrl + alt + ↓
		...
		try {
			...
			//while 문을 사용해서 이미지를 0부터 끝까지 가지고옴
			while ((길이 = 파일읽기.read(buffer)) > 0) {
						//파일 길이 = 파일읽기를 buffer를 통해 읽은 것이 0보다 크면
