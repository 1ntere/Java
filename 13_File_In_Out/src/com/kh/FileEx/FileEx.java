...
메서드
	...
	boolean mkdirs()		: 경로로 지정해준 모든 폴더 생성
							  ex) com.khc.FileEx하면 한번에 여러 개의 폴더가 생성됨
...
OS에 따라 폴더를 부르는 이름이 다름
	윈도우	│	리눅스 맥북
	폴더		│	디렉토리
			│
- 경로 표기 방법(파일 위치 표시도 약간 다름
	\		│	/
- java 언어는 /, \ 관계 없이 모두 경로로 표시 가능하게 설정이 되어 있음
...
public class FileEx {
	public static void main(String[] args) {
		...
		//존재하지 않는다면 폴더가 존재하지 않습니다. 출력하고 새로 만들어주기
		if (폴더만들기.exists()) {
