...
public class ScannerEx1 {
	...
	public static void main(String[] args) {
		...
		import 정확한 표기 범 : import java.util.Scanner;
		import 는 가져온다는 의미
		...
		*/
		...
		int num1 = sc.nextInt(); // 다음에 입력된 정수를 가져와 num1에 대입
		...
		// 단어 입력 2개 하기 next를 사용해서 변수명 word1 word2
		System.out.print("단어 입력 2개 하기 :");
		String word1 = sc.next();
		String word2 = sc.next();
		
		// Scanner를 이용해서 입력을 진행할 경우
		// System.out.print를 이용해서 값이 제대로 입력됐는지
		// 문제는 없는지 확인 출력을 해주는 습관을 들이는 것이 좋음
		...
		// 이유 : next() 뒤에 버퍼가 남는데 이게 바로 다음 코드에 입력이 되어서 바로 다음 코드가 진행됨
		sc.nextLine();
		//next() 뒤에 남는 버퍼값을 Line 안에 넣어줌
		// 버퍼값은 우리가 눈으로 보거나 저장을 해야 할 이유가 없기 때문에
		// 앞에 자료형이나 변수명을 사용해서 버퍼를 넣어 저장할 이유가 없음
		// 따라서 sc.nextLine() 으로만 사용
				
		System.out.print("문장 입력 : ");
		String str = sc.nextLine();
		
		
		// 4. System.out을 이용해서 키보드에 작성된 내용 출력하기
